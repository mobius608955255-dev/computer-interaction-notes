(function () {
  ...
  const toolbar = (app, active, commands = '') => {
    ...
    return `<div class="office-ribbon" aria-label="${app} 2016 功能区">
    <div class="office-tabs"><span>文件</span>${tabs.map(tab => `<span class="${tab === active ? 'active' : ''}">${tab}</span>`).join('')}</div>
    <div class="office-commands">${commands}</div>
  </div>`;
  };
  const office = (app, title, activeTab, commands, canvas, side = '') => `<div class="office-window app-${app.toLowerCase()}">
    <div class="office-titlebar"><i></i><strong>${escapeHTML(title)} - ${app} 2016</strong><span>—　□　×</span></div>
    ${toolbar(app, activeTab, commands)}
  ...
  const scenes = {
    ...
    y2026q31(demo) {
      return `<div class="compute-arena">
        <div class="processor cpu-board"><header><b>CPU</b><span>4 个复杂核心</span></header><div>${Array.from({length:4},(_,i)=>`<i style="--i:${i}"></i>`).join('')}</div><small>分支 · 调度 · 通用控制</small></div>
        <div class="workload-queue">${getItems(demo).map((item,i)=>`<button type="button" data-sim-choice="${i}"><span>${['系统调度','矩阵运算','操作系统'][i]}</span><i>拖入处理器</i></button>`).join('')}</div>
        <div class="processor gpu-board"><header><b>GPU</b><span>大量并行单元</span></header><div>${Array.from({length:48},(_,i)=>`<i style="--i:${i}"></i>`).join('')}</div><small>同类任务 · 大规模并行</small></div>
    ...
    y2020q2(demo) {
      return win('桌面', `<div class="window-playground"><button type="button" data-sim-choice="0" class="mini-window window-a"><span>记事本</span><textarea aria-label="记事本内容">键盘输入会到这里</textarea></button><button type="button" data-sim-choice="0" class="mini-window window-b"><span>计算器</span><b>128</b></button>${choice(demo,1,'window-minimize')}${choice(demo,2,'window-drag')}</div>`)+feedback('点击任一窗口，它会来到最前并获得输入焦点；可见、运行、活动是三个不同状态。');
    },
    y2026q32(demo) {
      return `<div class="power-scene"><div class="desktop-wallpaper"><div class="start-menu"><header>Drd</header><div class="start-apps"><i>文档</i><i>设置</i><i>图片</i></div><footer><b>⏻</b>${getItems(demo).map((item,i)=>`<button type="button" data-sim-choice="${i}">${escapeHTML(item.label)}</button>`).join('')}</footer></div><div class="shutdown-overlay" data-shutdown-state><b>Windows</b><span>正在准备...</span></div></div></div>${feedback('规范关机会先通知程序、写回缓存并卸载文件系统；强制断电跳过这些步骤。')}`;
    },
    y2020q3(demo) {
    ...
    y2020q24(demo) {
      return win('文件资源管理器', `<div class="dual-explorer"><section><header>本地磁盘 (C:)</header><button class="drag-file" type="button"><i>DOCX</i><b>笔记.docx</b></button></section><div class="drag-modifiers">${getItems(demo).map((item,i)=>`<button type="button" data-sim-choice="${i}"><kbd>${['同盘','跨盘','Ctrl','Shift','右键'][i]}</kbd><span>${escapeHTML(item.label)}</span></button>`).join('')}</div><section><header>数据盘 (D:)</header><div class="drop-zone">把文件拖到这里</div></section></div>`)+feedback('源盘和目标盘决定默认动作；Ctrl、Shift和右键拖动可以明确覆盖默认规则。');
    },
    'merged-4'(demo) {
    ...
    y2026q25(demo) {
      return win('设置', `<div class="settings-scene"><aside>${step(demo,0,'个性化','settings-step settings-0')}${step(demo,1,'主题','settings-step settings-1')}${step(demo,2,'桌面图标设置','settings-step settings-2')}</aside><main><header>个性化</header><div class="settings-preview"><div class="desktop-preview"><i data-thispc>此电脑</i><span>回收站</span></div></div><div class="desktop-icons-dialog"><b>桌面图标</b>${step(demo,3,'☐ 计算机　　应用','desktop-apply')}</div></main></div>`)+feedback('按Windows 10的真实层级进入设置；最后勾选“计算机”并应用，桌面才出现“此电脑”。');
    },
    y2026q35(demo) {
    ...
    y2020q41(demo) {
      const commands = `<div class="ribbon-group sequence-ribbon">${step(demo,0,'插入','ribbon-command tab-command')}${step(demo,1,'页眉 ▼','ribbon-command header-command')}${step(demo,2,'键入：山东专升本计算机','ribbon-command type-command')}${step(demo,3,'关闭页眉和页脚','ribbon-command close-command')}</div>`;
      return office('Word','考试说明.docx','插入',commands,`<div class="word-page header-page"><div class="header-zone" data-header-zone><span data-header-text>双击或使用“插入→页眉”进入此区域</span></div><div class="page-body"><h4>考试说明</h4><p>考生应在规定时间内完成所有操作。</p></div><div class="page-boundary">页眉距顶端 1.5 cm</div></div>`)+feedback('从“插入”进入页眉层，输入内容后必须退出页眉编辑，才能回到正文。');
    },
    'merged-7'(demo) {
      ...
      return office('Word','通知.docx','开始',commands,`<div class="word-page formatting-page"><div class="selection-mark"><h4 data-format-title>关于开展计算机技能竞赛的通知</h4></div><p>各班级：</p><p>为提高同学们的计算机应用能力，现组织技能竞赛。</p><div class="format-scope"><span>字符格式：字号、字形</span><span>段落格式：对齐、缩进、间距</span></div></div>`)+feedback('标题看起来是一行文字，但“居中”改变的是它所在的整个段落。');
    },
    y2020q43(demo) {
    ...
    y2020q61(demo) {
      ...
      return office('Word','实验报告.docx','布局',commands,`<div class="orientation-stage"><article class="mini-page portrait"><b>第 1 节</b><p>正文</p></article><button type="button" data-sim-step="2" class="mini-page portrait target-page"><b>第 2 节</b><p>宽表格</p><small>点击放置光标</small></button><article class="mini-page portrait"><b>第 3 节</b><p>正文</p></article><div class="section-scissors"><span data-break-before>前边界未建立</span><span data-break-after>后边界未建立</span></div></div>`)+feedback('纸张方向属于“节”的页面设置；先把目标页用两个分节符单独围起来。');
    },
    y2026q47(demo) {
      ...
      return office('Word','中西文混排.docx','开始','',`<div class="word-page cjk-page"><div class="cjk-measure"><i>可用行宽</i></div><p data-cjk-paragraph>医学AI（Artificial Intelligence）在2026年进入临床研究的新阶段。</p><div class="line-counter"><b data-line-count>3</b><span>行</span></div><div class="cjk-other-actions">${getItems(demo).slice(1).map((item,i)=>`<button type="button" data-sim-choice="${i+1}">${escapeHTML(item.label)}</button>`).join('')}</div></div>`,commands)+feedback('取消自动中西文/数字间距后，字符占宽减少，行数可能改变；文字内容并没有被删除。');
    },
    y2020q60(demo) {
      const commands = `<div class="table-tools">${step(demo,1,'表格工具·设计','ribbon-command')}${step(demo,2,'边框 ▼','ribbon-command')}${step(demo,3,'所有框线','ribbon-command all-borders')}</div>`;
      return office('Word','成绩表.docx','设计',commands,`<div class="word-page table-border-page"><button type="button" data-sim-step="0" class="table-select-handle" aria-label="选中整张表">✥</button><table data-border-table><thead><tr><th>姓名</th><th>计算机</th><th>高数</th></tr></thead><tbody><tr><td>王宁</td><td>92</td><td>88</td></tr><tr><td>李悦</td><td>86</td><td>94</td></tr></tbody></table><div class="border-gallery"><span>下框线</span><span>外侧框线</span><b>所有框线</b></div></div>`)+feedback('先通过左上角移动手柄选中整表，再在表格工具“设计”中应用“所有框线”。');
    },
    y2020q62(demo) {
      const commands = `<div class="table-tools">${step(demo,1,'表格工具·布局','ribbon-command')}${step(demo,2,'重复标题行','ribbon-command repeat-header')}</div>`;
      const rows = Array.from({length:8},(_,i)=>`<tr><td>${String(i+1).padStart(2,'0')}</td><td>学生 ${i+1}</td><td>${80+i}</td></tr>`);
      return office('Word','长成绩表.docx','布局',commands,`<div class="two-paper-table"><article><table><thead><tr><th>序号</th><th>姓名</th><th>成绩</th></tr></thead><tbody>${rows.slice(0,4).join('')}</tbody></table></article><article><table><thead data-repeated-header><tr><th>序号</th><th>姓名</th><th>成绩</th></tr></thead><tbody>${rows.slice(4).join('')}</tbody></table></article><button type="button" data-sim-step="0" class="select-header-row">点击选中首行标题</button></div>`)+feedback('重复标题只在表格自然跨页时显示；手工复制一行不是同一功能。');
    },
    y2020q63(demo) {
      const commands = getItems(demo).map((item,i)=>`<button type="button" data-sim-choice="${i}" class="ribbon-command"><i>${['▤','↔','✥'][i]}</i><span>${escapeHTML(item.label)}</span></button>`).join('');
      return office('Word','数据表.docx','布局',commands,`<div class="word-page table-align-page"><table data-align-table><tbody><tr><td data-align-cell>姓名</td><td>成绩</td></tr><tr><td>王宁</td><td>92</td></tr></tbody></table><div class="page-centerline"></div><div class="alignment-readout"><span data-table-align>整表：左对齐</span><span data-cell-align>格内文字：左上</span></div></div>`)+feedback('整表相对页面居中，与单元格内容在格内居中，是两个作用对象。');
    },
    'merged-9'(demo) {
      const commands = `<div class="picture-size-box"><label>高度 <input value="4.00 cm" data-image-height></label><label>宽度 <input value="6.00 cm" data-image-width></label><label><input type="checkbox" checked data-aspect-lock> 锁定纵横比</label>${getItems(demo).map((item,i)=>`<button type="button" data-sim-choice="${i}">${escapeHTML(item.label)}</button>`).join('')}</div>`;
      return office('Word','实验图.docx','布局','',`<div class="word-page picture-page"><div class="selected-picture" data-picture><div class="photo-art">CT<br><small>800 × 600</small></div>${['nw','ne','sw','se'].map(x=>`<i class="resize-handle ${x}"></i>`).join('')}</div><div class="crop-mask" data-crop-mask></div><span class="picture-readout" data-picture-readout>6.00 × 4.50 cm · 4:3</span></div>`,commands)+feedback('锁定比例时改一边，另一边随比例联动；裁剪是改变可见区域，不是拉伸像素。');
    },
    y2026q36(demo) {
      const commands = `<div class="arrange-tools">${step(demo,2,'组合 ▼ → 组合','ribbon-command group-command')}</div>`;
      return office('Word','封面设计.docx','格式',commands,`<div class="word-page grouping-page"><button type="button" data-sim-step="0" class="group-object photo-object"><i>图片</i><small>嵌入型</small></button><button type="button" data-sim-step="1" class="group-object shape-object"><i>形状</i></button><button type="button" data-sim-step="1" class="group-object wordart-object"><i>医学 × AI</i></button><div class="group-boundary" data-group-boundary></div></div>`)+feedback('嵌入型图片像一个文字字符，先改为环绕型，才能与浮动形状一起多选并组合。');
    },
    'merged-6'(demo) {
      ...
      return office('Word','论文.docx','引用',commands,`<div class="word-page caption-page"><p>如<span class="cross-reference" data-cross-ref>图 ?</span>所示，模型准确率随训练轮次提高。</p><button type="button" data-sim-step="0" class="figure-object"><div class="mini-chart"><i></i><i></i><i></i></div><span data-caption>（尚未插入题注）</span></button><button type="button" data-sim-step="2" class="text-caret">将光标放到这里引用图号</button><div class="caption-dialog" data-caption-dialog><b>题注：图 1 模型准确率</b><small>标签：图　编号：1, 2, 3…</small></div></div>`)+feedback('题注负责生成和管理图号，交叉引用负责在正文中引用这个动态编号。');
    },
    ...
    y2020q7(demo) {
      ...
      return office('Excel','销售数据.xlsx','开始',commands,`<div class="excel-sheet hash-sheet"><div class="formula-bar"><b>fx</b><span>2026/09/02 10:28</span></div><div class="excel-grid two-cols"><b class="col-head">A</b><button type="button" data-column-handle class="col-head narrow">B ↔</button><span>日期</span><strong data-hash-cell>########</strong><span>金额</span><em>1280.00</em></div><div class="cell-tip">实际内容仍在，只是列宽容不下当前显示格式。</div></div>`)+feedback('加宽B列后日期会正常显示；清除内容会丢数据，不能当作“修复显示”。');
    },
    y2020q8(demo) {
      return office('Excel','工作簿1.xlsx','开始','',`<div class="excel-sheet sheet-delete"><div class="empty-grid">当前工作簿</div><div class="sheet-tabs" data-sheet-tabs><button class="active">Sheet1</button><button>Sheet2</button><i>＋</i></div><div class="sheet-context">${getItems(demo).map((item,i)=>`<button type="button" data-sim-choice="${i}">${escapeHTML(item.label)}</button>`).join('')}</div><div class="excel-dialog" data-excel-dialog><b>Microsoft Excel</b><p>工作簿内至少含有一张可见工作表。</p><button>确定</button></div></div>`)+feedback('删除Sheet2可以成功；只剩Sheet1时再次删除，Excel会阻止操作。');
    },
    y2020q48(demo) {
      const commands = `<div class="excel-number-tools">${getItems(demo).map((item,i)=>`<button type="button" data-sim-choice="${i}">${escapeHTML(item.label)}</button>`).join('')}</div>`;
      const rows = Array.from({length:6},(_,i)=>`<span>${i+2}</span><b data-fill-row="${i}">${i===0?'20260001':''}</b><em>${['张琳','王宁','李悦','赵飞','周然','陈安'][i]}</em>`).join('');
      return office('Excel','学生信息.xlsx','开始',commands,`<div class="excel-sheet fill-sheet"><div class="formula-bar"><b>fx</b><span data-fill-formula>'20260001</span></div><div class="excel-grid fill-grid"><b></b><b>A 学号</b><b>B 姓名</b>${rows}</div><button type="button" data-fill-handle aria-label="拖动填充柄"></button><div class="fill-options" data-fill-options><span>复制单元格</span><b>填充序列</b></div></div>`)+feedback('长编号先按文本安全输入；是否递增取决于样本和填充选项，不是“文本一定重复”。');
    },
    y2026q49(demo) {
      const commands = getItems(demo).map((item,i)=>`<button type="button" data-sim-choice="${i}" class="ribbon-command">${escapeHTML(item.label)}</button>`).join('');
    ...
    y2020q52(demo) {
      const commands = `<div class="conditional-tools">${step(demo,1,'条件格式 ▼','ribbon-command')}${step(demo,2,'突出显示单元格规则 → 小于…','ribbon-command')}${step(demo,3,'数值：60　浅红填充','ribbon-command')}</div>`;
      return office('Excel','成绩表.xlsx','开始',commands,`<div class="excel-sheet conditional-sheet"><table><tr><th>姓名</th><th>成绩</th></tr>${[['王宁',92],['李悦',58],['张琳',46],['赵飞',76]].map((r,i)=>`<tr><td>${r[0]}</td><td class="score-${r[1]}" data-score>${r[1]}</td></tr>`).join('')}</table><button type="button" data-sim-step="0" class="range-selector">选中 B2:B5</button><div class="conditional-dialog"><b>小于</b><input value="60" readonly><span>浅红填充深红色文本</span></div></div>`)+feedback('条件格式保留原数值，只根据规则改变外观；分数变化后格式会自动重算。');
    },
    y2020q9(demo) {
    ...
    y2020q49(demo) {
      ...
      return office('Excel','报名表.xlsx','数据',commands,`<div class="excel-sheet duplicates-sheet"><table data-duplicate-table><tr><th><button type="button" data-sim-step="0">学号</button></th><th>姓名</th><th>电话</th></tr><tr><td>20260018</td><td>王宁</td><td>138…01</td></tr><tr class="duplicate-row"><td>20260018</td><td>王宁</td><td>139…99</td></tr><tr><td>20260019</td><td>李悦</td><td>137…28</td></tr></table><div class="remove-duplicates-dialog"><b>删除重复项</b><label><input type="checkbox" checked disabled> 学号</label><label><input type="checkbox" disabled> 姓名</label><label><input type="checkbox" disabled> 电话</label>${step(demo,2,'只勾选“学号”','dialog-command')}${step(demo,3,'确定','primary-command')}</div></div>`)+feedback('选哪些列，就按哪些列的组合判重；这里只勾“学号”才符合题意。');
    },
    y2020q58(demo) {
      const commands = `<div class="data-tools">${step(demo,0,'排序：按班级','ribbon-command')}${step(demo,1,'分类汇总','ribbon-command')}</div>`;
      return office('Excel','班级成绩.xlsx','数据',commands,`<div class="excel-sheet subtotal-sheet"><table data-subtotal-table><tr><th>班级</th><th>姓名</th><th>成绩</th></tr><tr><td>2班</td><td>李悦</td><td>86</td></tr><tr><td>1班</td><td>王宁</td><td>92</td></tr><tr><td>2班</td><td>赵飞</td><td>76</td></tr><tr><td>1班</td><td>张琳</td><td>88</td></tr></table><div class="subtotal-dialog"><label>分类字段 <select><option>班级</option></select></label><label>汇总方式 <select><option>平均值</option></select></label>${step(demo,2,'班级＋平均值','dialog-command')}${step(demo,3,'成绩项＋确定','primary-command')}</div><div class="outline-levels">1　2　3</div></div>`)+feedback('分类汇总只会在相邻同类记录之间插入汇总，所以必须先按分类字段排序。');
    },
    y2026q51(demo) {
      const commands = getItems(demo).map((item,i)=>`<button type="button" data-sim-choice="${i}" class="ribbon-command">${escapeHTML(item.label)}</button>`).join('');
    ...
    y2026q52(demo) {
      const commands = `<div class="chart-tools">${step(demo,1,'插入柱形图','ribbon-command')}${step(demo,2,'更改图表类型 → 组合图','ribbon-command')}${step(demo,3,'增长率：次坐标轴','ribbon-command')}</div>`;
      return office('Excel','经营分析.xlsx','插入',commands,`<div class="excel-sheet combo-chart-sheet"><table><tr><th>月份</th><th>销售额/万元</th><th>增长率/%</th></tr><tr><td>1月</td><td>120</td><td>3</td></tr><tr><td>2月</td><td>180</td><td>7</td></tr><tr><td>3月</td><td>240</td><td>5</td></tr></table><button type="button" data-sim-step="0" class="select-data-range">选中 A1:C4</button><div class="combo-chart"><div class="primary-axis">240<br>120<br>0</div><div class="combo-bars"><i style="--h:40%"></i><i style="--h:62%"></i><i style="--h:82%"></i><svg viewBox="0 0 240 100" preserveAspectRatio="none"><polyline points="10,75 120,25 230,48"/></svg></div><div class="secondary-axis" data-secondary-axis>10%<br>5%<br>0%</div></div>${step(demo,4,'补充轴标题与单位','axis-title-command')}</div>`)+feedback('销售额和增长率量纲悬殊；给增长率设置次坐标轴，二者趋势才同时可读。');
    },

    // 第5章：还原 PowerPoint 的缩略图、幻灯片画布、窗格和放映行为。
    y2020q10(demo) {
      const commands = getItems(demo).map((item,i)=>`<button type="button" data-sim-choice="${i}" class="ribbon-command"><i>${['▦','▣','▶'][i]}</i><span>${escapeHTML(item.label)}</span></button>`).join('');
      return office('PowerPoint','医学AI汇报.pptx','视图',commands,`<div class="ppt-view-stage"><aside class="ppt-thumbnails">${Array.from({length:6},(_,i)=>`<span>${i+1}<i style="--slide:${i}"></i></span>`).join('')}</aside><div class="ppt-main-slide" data-ppt-view><small>01</small><h4>人工智能辅助医学影像</h4><div class="ppt-hero-chart"><i></i><i></i><i></i></div><p>课程汇报</p></div><div class="slide-sorter" data-slide-sorter>${Array.from({length:6},(_,i)=>`<button type="button"><i style="--slide:${i}"></i><span>${i+1}</span></button>`).join('')}</div></div>`)+feedback('幻灯片浏览视图把全部页面平铺，最适合整体重排；普通视图适合编辑单页。');
    },
    y2020q12(demo) {
      const commands = getItems(demo).map((item,i)=>`<button type="button" data-sim-choice="${i}" class="ribbon-command">${escapeHTML(item.label)}</button>`).join('');
      return office('PowerPoint','答辩.pptx','幻灯片放映',commands,`<div class="ppt-hide-stage"><aside class="ppt-thumbnails">${[1,2,3,4].map(i=>`<button type="button" class="thumb-${i}"><i>${i}</i><span>第${i}页</span></button>`).join('')}</aside><div class="ppt-main-slide"><h4 data-slide-title>第 3 页：备用数据</h4><div class="hidden-stamp" data-hidden-stamp>隐藏</div><p>页面仍保留在文件中，只在正常放映时被跳过。</p></div><div class="play-route"><span>1</span><span>2</span><i>跳过 3</i><span>4</span></div></div>`)+feedback('隐藏是放映属性，删除才会从演示文稿中移除页面。');
    },
    y2026q53(demo) {
      const commands = getItems(demo).map((item,i)=>`<button type="button" data-sim-choice="${i}" class="ribbon-command">${escapeHTML(item.label)}</button>`).join('');
      return office('PowerPoint','病例展示.pptx','格式',commands,`<div class="ppt-picture-stage"><aside class="animation-pane"><b>动画窗格</b><span><i>1</i> 图片：淡入</span><span><i>2</i> 标题：浮入</span></aside><div class="ppt-main-slide"><div class="ppt-selected-picture" data-ppt-picture><div class="scan-image">CT<br><small>原图</small></div>${['nw','ne','sw','se'].map(x=>`<i class="resize-handle ${x}"></i>`).join('')}<span class="picture-effect">柔化边缘 5 磅</span></div><h4>影像学表现</h4></div><div class="picture-source-gallery"><span>MRI 新图</span><span>本地文件</span><span>剪贴板</span></div></div>`)+feedback('“更改图片”保留原对象身份，所以位置、大小、边框和动画大多能继续保留。');
    },
    y2026q54(demo) {
      ...
      return office('PowerPoint','课程提纲.pptx','开始',commands,`<div class="ppt-autofit-stage"><div class="ppt-main-slide"><div class="text-placeholder" data-autofit-box><h4>学习目标</h4><p data-autofit-text>掌握计算机基础概念；理解操作系统的基本功能；能够在Word、Excel和PowerPoint中完成规范操作；形成信息安全意识。</p><button class="autofit-smarttag">↙</button></div></div><div class="overflow-meter"><b data-overflow-value>超出 32%</b><i></i><small>占位符边界固定</small></div></div>`)+feedback('自适应不是只有一种：可以缩小文字、扩大形状，或把溢出内容拆到新幻灯片。');
    },
    y2020q11(demo) {
    ...
    'merged-11'(demo) {
      return office('PowerPoint','交互课件.pptx','动画',`<div class="motion-tabs"><span>切换</span><span>动画</span><span>动作</span></div>`,`<div class="ppt-motion-stage"><aside class="motion-timeline"><b>时间轴</b><i data-motion-marker></i><span>0s</span><span>5s</span></aside><div class="ppt-main-slide"><button type="button" data-sim-choice="1" class="animated-title">医学AI导论</button><button type="button" data-sim-choice="2" class="action-button">跳到第8页</button><div class="slide-transition" data-transition-layer></div></div><div class="motion-controls">${choice(demo,0)}${choice(demo,3)}</div></div>`)+feedback('切换作用于整页之间；动画作用于对象；动作决定点击后跳到哪里。');
    },
    'merged-12'(demo) {
      const commands = `${step(demo,2,'添加动画：擦除','ribbon-command')}${step(demo,3,'效果选项：按类别','ribbon-command')}`;
      return office('PowerPoint','数据汇报.pptx','动画',commands,`<div class="ppt-chart-animation"><aside class="animation-pane"><b>动画窗格</b><span data-animation-item>1　图表：擦除</span>${step(demo,4,'▶ 预览','preview-command')}</aside><div class="ppt-main-slide"><div class="ppt-chart" data-ppt-chart>${[['一班',72],['二班',88],['三班',63]].map((x,i)=>`<div class="ppt-bar bar-${i}"><i style="--h:${x[1]}%"></i><span>${x[0]}</span></div>`).join('')}</div><div class="paste-source">${step(demo,0,'从Excel复制图表','source-command')}${step(demo,1,'保留图表结构粘贴','source-command')}</div></div></div>`)+feedback('只有保留图表结构，PowerPoint才认识系列与类别，才能分组依次播放。');
    },
    y2026q55(demo) {
      const commands = `${step(demo,0,'自定义幻灯片放映','ribbon-command')}`;
      return office('PowerPoint','答辩.pptx','幻灯片放映',commands,`<div class="custom-show-dialog"><header>自定义放映</header><div class="show-list"><section><b>演示文稿中的幻灯片</b>${[1,2,3,4,5].map(i=>`<span>第${i}页</span>`).join('')}</section><div class="show-arrows">›<br>‹</div><section><b>答辩精简版</b>${[1,2,3,4].map(i=>`<button type="button" ${i===3?'data-sim-step="2"':''}>第${i}页</button>`).join('')}</section></div><div class="custom-show-actions">${step(demo,1,'编辑方案','dialog-command')}${step(demo,3,'移除并确定','primary-command')}</div><div class="slide-file-count">文件中仍有 <b>5</b> 张幻灯片</div></div>`)+feedback('从右侧自定义放映列表移除第3页，只改变该放映方案；源文件仍保留5页。');
    },
    ...
    y2020q14(demo) {
      return `<div class="er-designer"><div class="entity branch"><b>团支部</b><span><i>🔑</i> 支部编号</span><span>支部名称</span></div><div class="relationship-line"><strong>1</strong><i></i><strong>∞</strong></div><div class="entity member"><b>团员</b><span><i>🔑</i> 学号</span><span>姓名</span><span class="foreign-key">支部编号（外键）</span></div><div class="er-actions">${getItems(demo).map((item,i)=>`<button type="button" data-sim-choice="${i}">${escapeHTML(item.label)}</button>`).join('')}</div><div class="record-validator" data-record-validator>新增团员时，支部编号必须引用已存在的支部。</div></div>${feedback('从支部看是“一对多”，从团员看是“多对一”；外键通常放在多端。')}`;
    },
    y2026q12(demo) {
