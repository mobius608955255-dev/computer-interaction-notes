(() => {
  ...
  // 2/3 章原先使用 root.innerHTML 局部换页，1/4 章仍由 React 页面接管。
  // 跨章统一采用完整页面导航，避免两套路由和观察器互相覆盖导致卡死。
  ...
  function sortChapterNavigation() {
    $$('nav').forEach(nav => {
      ...
      const ordered = [...links].sort((a, b) => Number(a.dataset.chapter) - Number(b.dataset.chapter));
      const alreadyOrdered = ordered.every((a, i) => links[i] === a);
  ...
  function buildCatalogueDetails(sections) {
    ...
    details.addEventListener('click', event => {
      ...
      const chapter = currentChapter();
      const sectionHash = a.getAttribute('href');
      history.replaceState({}, '', `./?v=6&chapter=${chapter}${sectionHash}`);
    });
    return details;
  }
  ...
  // v4 仍会生成掌握度与练习相关 DOM；保留其目录检索能力，但去掉练习/掌握语义。
  function neutralizePracticeChrome() {
    $$('.study-hub-trigger > span').forEach(el => {
      if (el.textContent !== '目录') el.textContent = '目录';
    });
    const title = $('#study-hub-title');
    if (title && title.textContent !== '快速目录') title.textContent = '快速目录';
    const directoryTab = $('[data-study-tab="directory"]');
    if (directoryTab && directoryTab.textContent !== '本章目录') directoryTab.textContent = '本章目录';
    const search = $('#study-search');
    if (search && search.placeholder !== '搜索本章知识点') search.placeholder = '搜索本章知识点';
    $$('#study-directory [data-go-section] i').forEach(i => {
