...
状态：v29发布候选；2021已接入，本记录保留原问题清单用于复核。

## 已完成读取

- 2021：题目10页、答案13页，60题全部逐页视觉核读。
- 新建 `notes-2021-data.js`：60题映射，16条新增知识笔记，其余并入已有条目；接入后预期220条笔记、460个来源（尚须测试）。
- 新建 `note-labs-2021.js`：16个演示初稿，尚未接入、未验收。文本框需要在基础手势管理器中接入 box 拖动；CSV数值推断应改为明确模拟15位Excel精度；仍须逐场景验证。
- `notes-data.js` 已增加可选2021归并；旧页面未加载2021脚本，仍正常使用旧数据。`note-labs.js` 导出注册器与UI助手，并修正行恢复后被无关命令再次隐藏的问题。
- 原有400题来源连续唯一；原有19项测试通过（检查前）。204条笔记均能打开/重置/收起，但不等于内容与交互全部正确。
...
- 待原题复核2026q47：中文与数字、中文与西文是独立设置；不得猜题意。
- 待原题复核2026q51：区分自动筛选与原位置高级筛选的“数据→清除”。
...
浏览器逐场景抽检与窄屏测量在发布后记录；源题全覆盖不等于全考纲或完整Office功能覆盖。
