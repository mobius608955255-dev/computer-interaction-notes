...
[打开网站](https://mobius608955255-dev.github.io/computer-interaction-notes/?v=58)
...
v58已由Work2发布并完成桌面、390px终验。Work1已完成v59“Excel格式与数据管理分析”本地开发与冻结，尚未发布；本轮真实浏览器终验交由Work2在Pages部署后完成。Excel专项与恢复入口见[CONTENT-EXCEL.md](CONTENT-EXCEL.md)；既往Word及第一章记录保留在[CONTENT-ROUND2.md](CONTENT-ROUND2.md)。
...
| `content/legacy-demos.json` | 保留的旧场景步骤或选择元数据；升级为状态模型时删除对应旧定义 |
...
2. `pointGroups.indices`须覆盖该笔记每个要点且恰好一次。已有 `points` 的 0 基索引属于历史链接契约，展示优化不得插入、删除或重排；优先使用 `pointPresentation` 标题、原文分界和导航元数据。确需改变原语义位置时须先建立兼容映射并验证旧链接。关联必须指向真实ID并写明阅读目的，比较表每行应引用原文。构建验证链接、分组与对照结构。
...
1. 先核实实际可读取的来源和完整相关范围。ZIP检查包内文件，图片、表格和批注查看原图；“定位到文件”“读到片段”和“核查相关完整范围”分别记录。年份、课次、软件版本不能靠名称猜测。
2. 考纲定边界，课件与转写还原课堂覆盖，原卷连同选项确定做题所需细节。AI笔记和旧统计矩阵只作候选；保留正确解释、纠正错误，缺证据就明确待核。不把未署名参考答案称为官方答案。
3. 定义应讲清对象、区别与边界；操作应讲清前提、入口、步骤、结果与限制；规则应有完整例子和反例。无需给每条堆固定栏目，也不以“七年没考”删除必要知识。“所有、只能、一定、不能撤销”等表述必须限定对象、版本和条件。
4. 正文唯一维护在 `content/chapterN.json`。沿用稳定知识ID与已有段落锚点，细节优先追加并分组；变更既有收藏、错题、进度关联前先设计兼容方案。私有逐点来源映射是审计快照，不是另一份可独立编辑的知识库。
5. 内容验收与代码验收分开。构建、测试和卡片数量不证明知识完整；网页模型通过测试不等于真实Windows/Office实测。不能为通过测试删有效断言或降低检查标准。
